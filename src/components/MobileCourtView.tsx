...
export default function MobileCourtView({ 
  ...
  {/* TIMING DOUBLE STOPWATCH UNIT */}
      ...
      <div className="px-4 py-2 bg-slate-900 border-b border-slate-800 flex items-center justify-between gap-2 shrink-0 overflow-x-auto no-scrollbar">
        <button
          type="button"
          onClick={() => setShowQuickSessionModal(true)}
          className="px-2.5 py-1.5 bg-orange-600 hover:bg-orange-500 text-white text-[10px] font-black uppercase tracking-wider rounded-lg transition flex items-center gap-1 cursor-pointer active:scale-95 shadow-md shrink-0 border border-orange-400/30"
          title="Crear una nova sessió d'entrenament ràpida des del mòbil"
        >
          <Plus size={13} strokeWidth={3} />
          <span>+ Nova Sessió Mòbil</span>
        </button>
      ...
      {/* QUICK MOBILE SESSION CREATOR MODAL */}
      {showQuickSessionModal && (
        <div className="fixed inset-0 bg-slate-950/80 z-[99999] flex items-center justify-center p-4 backdrop-blur-xs select-none animate-in fade-in duration-200">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 max-w-md w-full space-y-4 max-h-[90vh] overflow-y-auto shadow-2xl text-left">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                <span className="p-2 bg-orange-600/20 text-orange-400 rounded-xl text-base">⚡</span>
                <div>
                  <h3 className="text-sm font-black text-white uppercase tracking-wider font-mono">Nova Sessió Mòbil Ràpida</h3>
                  <p className="text-[10px] text-slate-400 font-sans">Crea un entrenament en 1 clic sense carregar la pantalla</p>
                </div>
              </div>
              <button
                onClick={() => setShowQuickSessionModal(false)}
            ...
            <QuickMobileSessionForm
              drills={drills}
              onClose={() => setShowQuickSessionModal(false)}
              onCreate={(newSession) => {
                if (onUpdateSession) onUpdateSession(newSession);
                if (onSelectSessionId) onSelectSessionId(newSession.id);
                setShowQuickSessionModal(false);
                triggerLocalToast("⚡ Sessió Ràpida creada i activada!");
              }}
            />
          </div>
        </div>
...
function QuickMobileSessionForm({
  drills,
  onClose,
  onCreate
}: {
  drills: Drill[];
  onClose: () => void;
  onCreate: (newSession: TrainingSession) => void;
}) {
  const [title, setTitle] = useState(`Sessió ${new Date().toLocaleDateString('ca-ES', { day: 'numeric', month: 'short' })}`);
  const [category, setCategory] = useState<'Sènior' | 'Júnior' | 'Cadet' | 'Infantil' | 'Mini'>('Júnior');
  const [targetDuration, setTargetDuration] = useState<number>(75);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    let scheduled: TrainingSession['drills'] = [];

    if (mode === 'auto') {
      const warmups = drills.filter(d => d.category === 'Escalfament');
      const attacks = drills.filter(d => d.category === 'Atac');
      const defenses = drills.filter(d => d.category === 'Defensa');
      const others = drills.filter(d => !['Escalfament', 'Atac', 'Defensa'].includes(d.category || ''));

      const picked: Drill[] = [];
      if (warmups.length > 0) picked.push(warmups[0]);
      if (attacks.length > 0) picked.push(attacks[0]);
      if (defenses.length > 0) picked.push(defenses[0]);
      if (attacks.length > 1) picked.push(attacks[1]);
      if (others.length > 0) picked.push(others[0]);

      let totalTime = picked.reduce((acc, d) => acc + (d.duration || 10), 0);
      for (const d of drills) {
        if (totalTime >= targetDuration) break;
        if (!picked.some(p => p.id === d.id)) {
          picked.push(d);
          totalTime += (d.duration || 10);
        }
      }

      scheduled = picked.map(d => ({
        drillId: d.id,
        duration: d.duration || 10,
        customNotes: ''
      }));
    } else {
      scheduled = selectedIds.map(id => {
    ...
    if (scheduled.length === 0 && drills.length > 0) {
      scheduled = drills.slice(0, 4).map(d => ({ drillId: d.id, duration: d.duration || 10, customNotes: '' }));
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4 font-sans text-left">
      <div>
        ...
        <input
          ...
          placeholder="ex: Entrenament de Dijous"
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <select
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-xl px-2.5 py-2 text-xs font-bold text-slate-200 focus:outline-none focus:border-orange-500"
          >
            <option value="Sènior">Sènior</option>
        ...
        <div>
          ...
          <select
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-xl px-2.5 py-2 text-xs font-bold text-orange-400 focus:outline-none focus:border-orange-500 font-mono"
          >
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            <span className="block text-[10px] font-black uppercase tracking-wider text-orange-400">⚡ Automàtic Equilibrat</span>
            <span className="text-[9px] font-normal text-slate-400 block mt-0.5">Selecció intel·ligent per durada</span>
          </button>
          ...
          >
            <span className="block text-[10px] font-black uppercase tracking-wider text-orange-400">📝 Selecció Manual</span>
            <span className="text-[9px] font-normal text-slate-400 block mt-0.5">Tria de la llista d'exercicis</span>
          </button>
        </div>
      </div>
            ...
            {filteredDrills.map((d) => {
              ...
              return (
                <div
                  key={d.id}
                  onClick={() => {
                    if (isSel) setSelectedIds(selectedIds.filter(id => id !== d.id));
                  ...
                  className={`p-2 rounded-lg border text-xs cursor-pointer flex items-center justify-between transition ${
                    isSel ? 'bg-orange-600/20 border-orange-500 text-white' : 'bg-slate-950 border-slate-800 text-slate-300'
                  }`}
                >
                  <div className="min-w-0 flex-1">
                    <span className="font-bold truncate block">{d.title}</span>
                    <span className="text-[9px] text-slate-400 font-mono">{d.category} • {d.duration}′</span>
                  </div>
                  <input
                    type="checkbox"
                    checked={isSel}
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-slate-800">
        ...
        <button
          type="submit"
          className="flex-1 py-2 bg-orange-600 hover:bg-orange-500 text-white text-xs font-black uppercase tracking-wider rounded-xl transition shadow-lg shadow-orange-600/20 cursor-pointer"
        >
          ⚡ Crear i Començar
        </button>
      </div>
