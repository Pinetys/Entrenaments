            ...
            <div className="flex flex-wrap items-center gap-1.5 shrink-0 select-none">
              <div className="relative">
                <button
                  ...
                  title="Copiar tots els exercicis d'aquesta sessió"
                >
                  <Copy size={13} /> Duplicar
                </button>

                {showDuplicateDropdown && (
                  <div id="duplicate-session-dropdown" className="absolute right-0 top-full mt-2 w-72 bg-white border border-slate-200 rounded-md shadow-lg p-3 z-[100]">
                    <div className="flex items-center justify-between mb-2 pb-1 border-b border-slate-100">
                      <span className="text-[10px] uppercase font-black text-slate-400">Duplicar Sessió Activa</span>
                      <button onClick={() => setShowDuplicateDropdown(false)} className="text-slate-400 hover:text-slate-600">
                        <X size={12} />
                      </button>
                    </div>
                      ...
                      .map(target => {
                        ...
                        return (
                          <button
                            ...
                            className="w-full text-left px-2 py-1 bg-white hover:bg-orange-50 font-medium text-slate-700 border border-slate-100 flex items-center justify-between text-xs transition cursor-pointer rounded"
                          >
                            <span className="truncate">Sessió {target.num}: {destSession?.name || `S${target.num}`}</span>
                            <span className="text-[9px] font-mono text-slate-400">{drillsCount > 0 ? `${drillsCount} ex.` : 'Buda'}</span>
                          </button>
                        );
                      })}
                    </div>
            ...
            {drills
              .filter((drill) => {
                ...
                if (plannerCategoryFilter === 'Atac') {
                  return ['Atac'].includes(drill.category);
                }
                if (plannerCategoryFilter === 'Transició') {
                  return ['Transició'].includes(drill.category);
                }
                return drill.category === plannerCategoryFilter;
              })
              .map((drill) => {
